...
#
# --clean removes everything a build produces - dist/, release/,
# dist-pyinstaller/, build/, the Rust target/ and gen/ trees, copied
# sidecar binaries and __pycache__ - and exits. It leaves untracked
# local files alone, which `git clean -xdf` would not.
...
if [ "$DO_CLEAN" = "1" ]; then
    ...
    # Everything here is regenerated by a build.
    for path in dist dist-pyinstaller build release .pytest_cache \
    ...
    echo
    echo "Clean. Source and local settings are untouched."
    exit 0
fi
